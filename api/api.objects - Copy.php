...
function GetBoxesList($strStatus, $strBox, $strCustomer, $strOrderStatus, $token) {
	//Can query database and any other complex operation
	...
	if ($isValid==0) {
		...
		//$barcode = substr($barcode,6);
		if ($strStatus=="0") {
			$qry = "SELECT T1.wf_id as box, T3.company_name, T1.row_id as row_id, (T4.status) as status FROM workflow T1 INNER JOIN pickup T2 ON T2.fk_barcode = T1.wf_id INNER JOIN companies T3 ON T3.row_id = T2.fk_company INNER JOIN wf_status T4 ON T4.row_id = T1.fk_status WHERE T1.wf_id LIKE '%".$strBox."%' AND T3.company_name LIKE '%".$strCustomer."%' ORDER BY wf_id ASC";
		} else {
			$qry = "SELECT T1.wf_id as box, T3.company_name, T1.row_id as row_id, (T4.status) as status FROM workflow T1 INNER JOIN pickup T2 ON T2.fk_barcode = T1.wf_id INNER JOIN companies T3 ON T3.row_id = T2.fk_company INNER JOIN wf_status T4 ON T4.row_id = T1.fk_status WHERE T1.wf_id LIKE '%".$strBox."%' AND T3.company_name LIKE '%".$strCustomer."%' 	AND T1.fk_status IN (" . $strStatus . "," . ($strStatus + 1) . ") ORDER BY wf_id ASC";
		}
		//$qry = "SELECT * FROM objects WHERE fk_obj_type = 2 AND fk_parent = ".$strParent;
		$res = mysql_query($qry);
		...
		if ($num_rows>0) {
			while ($row = mysql_fetch_array($res)) {
				...
				$oReturn[$lCounter] = array('row_id'=>$row['row_id'], 'box'=>$row['box'], 'company_name'=>$row['company_name'], 'status'=>$row['status']);
				$lCounter = $lCounter+1;
			}
		} else {
			$oReturn[0] = array('row_id'=>'-1', 'status'=>"No Results");
		}
		return $oReturn;
	} else {
		$oReturn[0] = array('row_id'=>'-1', 'status'=>"Token Error");
		return $oReturn;
	}
...
function GetCharts($strParent, $token) {
	...
	$server->register("GetBoxesList",// method name
        array("strStatus" => "xsd:string", "strBox" => "xsd:string", "strCustomer" => "xsd:string", "strOrderStatus" => "xsd:string", "token" => "xsd:string"),// input parameter - nothing!
        array("return" => "tns:BoxArray"),// output - object of type MyTableArray.
        "urn:boxeswsdl",
       	"urn:boxeswsdl#GetBoxesList",
        "rpc",// style.. remote procedure call
        false,// use of the call
        "Get orders for the specified company."// documentation for people who hook into your service.
    );
