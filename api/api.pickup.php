...
function GetPatientName($barcode, $token) {
	...
	if ($isValid==0) {
		...
		$qry = "SELECT * FROM meta  WHERE row_id = ".$barcode;
		$res = mysql_query($qry);
	...
	$server->configureWSDL("GetPatientName", "urn:patient");
 
	//Register web service function so that clients can access
	$server->register("GetPatientName",
	array("barcode" => "xsd:string", "token" => "xsd:string"),
	array("return" => "xsd:string"),
	"urn:patient",
	"urn:patient#GetPatientName",
	"rpc",
	"encoded",
	"Get Patient Name");
