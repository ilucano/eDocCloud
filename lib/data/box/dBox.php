<?php
	...
	$qryCnt = "SELECT COUNT(*) as num FROM objects T1 INNER JOIN companies T2 ON T1.fk_company = T2.row_id WHERE T1.fk_obj_type = 2 ".$qryAdd;
	// FIN DEL CAMBIO
	...
	if ($total_pages>$limit || $pagAct > 0) {
		if ($pagAct==0) {
			$qryFT = "SELECT T1.*, (T2.company_name) as empresa, (T3.status) as status FROM objects T1 INNER JOIN companies T2 ON T1.fk_company = T2.row_id INNER JOIN ordstatus T3 ON T1.fk_status = T3.row_id WHERE fk_obj_type = 2 ".$qryAdd." LIMIT ".($pagAct * $limit).",".($limit).";";
		} else {
			$qryFT = "SELECT T1.*, (T2.company_name) as empresa, (T3.status) as status FROM objects T1 INNER JOIN companies T2 ON T1.fk_company = T2.row_id INNER JOIN ordstatus T3 ON T1.fk_status = T3.row_id WHERE fk_obj_type = 2 ".$qryAdd." LIMIT ".(($pagAct) * $limit).",".($limit).";";
		}
	} else {
		$qryFT = "SELECT T1.*, (T2.company_name) as empresa, (T3.status) as status FROM objects T1 INNER JOIN companies T2 ON T1.fk_company = T2.row_id INNER JOIN ordstatus T3 ON T1.fk_status = T3.row_id WHERE fk_obj_type = 2 ".$qryAdd.";";
	}
	// FIN DEL CAMBIO
