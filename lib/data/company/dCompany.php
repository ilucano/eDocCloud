<?php
	...
	// COMIENZO DEL CAMBIO
	if ($txtSearch!="") {
		$qryAdd = " WHERE company_name LIKE '%".$txtSearch."%' ";
	}
	$qryCnt = "SELECT COUNT(*) as num FROM companies ".$qryAdd;
	// FIN DEL CAMBIO
	...
	if ($total_pages>$limit || $pagAct > 0) {
		if ($pagAct==0) {
			$qryFT = "SELECT T1.*, CONCAT(T2.first_name, ' ',T2.last_name, ' (', T2.username, ')') as user FROM companies T1 INNER JOIN users T2 ON T1.fk_admin = T2.row_id ".$qryAdd." LIMIT ".($pagAct * $limit).",".($limit).";";
		} else {
			$qryFT = "SELECT T1.*, CONCAT(T2.first_name, ' ',T2.last_name, ' (', T2.username, ')') as user FROM companies T1 INNER JOIN users T2 ON T1.fk_admin = T2.row_id ".$qryAdd." LIMIT ".(($pagAct) * $limit).",".($limit).";";
		}
	} else {
		$qryFT = "SELECT T1.*, CONCAT(T2.first_name, ' ',T2.last_name, ' (', T2.username, ')') as user FROM companies T1 INNER JOIN users T2 ON T1.fk_admin = T2.row_id ".$qryAdd.";";
	}
	// FIN DEL CAMBIO
	...
	if (mysql_num_rows($res)) {
		// COMIENZO DEL CAMBIO
		echo "<table><tbody><thead><tr><th width=\"25%\">Company Name</th><th width=\"25%\">Company Address</th><th width=\"30%\">Administrator</th><th width=\"20%\">Actions</th></tr></thead>";
		
		while ($row = mysql_fetch_array($res)) {
			echo "<tr><td width=25%>";
			echo $row['company_name'].'</td>';
			echo '<td width="25%">'.$row['company_address1'].'<br>'.$row['company_address2'].'</td>';
			echo '<td width="30%">'.$row['user'].'</td>';
			// FIN DEL CAMBIO
			echo '<td width="10%">';
