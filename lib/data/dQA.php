<?php
	...
	$qryCnt = "SELECT COUNT(*) as num FROM workflow WHERE fk_status IN (8) OR (fk_status = 9 AND created_by = ".$_SESSION['Vid'].")";
	...
	if ($total_pages>$limit || $pagAct >= 0) {
		if ($pagAct==0) {
			$qryFT = "SELECT T1.*, (T2.status) as status, (SELECT company_name FROM companies WHERE row_id = T4.fk_company) as empresa, (T4.row_id) boxid FROM workflow T1 INNER JOIN wf_status T2 ON T1.fk_status = T2.row_id INNER JOIN pickup T3 ON T1.wf_id = T3.fk_barcode LEFT JOIN objects T4 ON T3.fk_box = T4.row_id WHERE T1.fk_status IN (8) OR (T1.fk_status = 9 AND T1.created_by = ".$_SESSION['Vid'].") ORDER BY T1.fk_status DESC, T3.fk_barcode LIMIT 0,".($limit).";";
		} else {
			$qryFT = "SELECT T1.*, (T2.status) as status, (SELECT company_name FROM companies WHERE row_id = T4.fk_company) as empresa, (T4.row_id) boxid FROM workflow T1 INNER JOIN wf_status T2 ON T1.fk_status = T2.row_id INNER JOIN pickup T3 ON T1.wf_id = T3.fk_barcode LEFT JOIN objects T4 ON T3.fk_box = T4.row_id WHERE T1.fk_status IN (8) OR (T1.fk_status = 9 AND T1.created_by = ".$_SESSION['Vid'].")  ORDER BY T1.fk_status DESC, T3.fk_barcode LIMIT ".((($pagAct) * $limit)).",".($limit).";";
		}
	} else {
		$qryFT = "SELECT T1.*, (T2.status) as status, (SELECT company_name FROM companies WHERE row_id = T4.fk_company) as empresa, (T4.row_id) boxid FROM workflow T1 INNER JOIN wf_status T2 ON T1.fk_status = T2.row_id INNER JOIN pickup T3 ON T1.wf_id = T3.fk_barcode LEFT JOIN objects T4 ON T3.fk_box = T4.row_id WHERE T1.fk_status IN (8) OR (T1.fk_status = 9 AND T1.created_by = ".$_SESSION['Vid'].")  ORDER BY T1.fk_status DESC, T3.fk_barcode;";
	}
