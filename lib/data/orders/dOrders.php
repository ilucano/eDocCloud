<?php
	...
	// COMIENZO DEL CAMBIO
	if ($txtSearch!="") {
		$qryAdd = " AND (T1.f_code LIKE '%".$txtSearch."%' OR T1.f_name LIKE '%".$txtSearch."%' OR T2.company_name LIKE '%".$txtSearch."%')";
	}
	$qryCnt = "SELECT COUNT(*) as num FROM objects T1 INNER JOIN companies T2 ON T1.fk_company = T2.row_id WHERE T1.fk_obj_type = 1 ".$qryAdd;
	// FIN DEL CAMBIO
	...
	if ($total_pages>$limit || $pagAct > 0) {
		if ($pagAct==0) {
			$qryFT = "SELECT T1.*, (T2.company_name) as empresa, (T3.status) as status FROM objects T1 INNER JOIN companies T2 ON T1.fk_company = T2.row_id INNER JOIN ordstatus T3 ON T1.fk_status = T3.row_id WHERE fk_obj_type = 1 ".$qryAdd." LIMIT ".($pagAct * $limit).",".($limit).";";
		} else {
			$qryFT = "SELECT T1.*, (T2.company_name) as empresa, (T3.status) as status FROM objects T1 INNER JOIN companies T2 ON T1.fk_company = T2.row_id INNER JOIN ordstatus T3 ON T1.fk_status = T3.row_id WHERE fk_obj_type = 1 ".$qryAdd." LIMIT ".(($pagAct) * $limit).",".($limit).";";
		}
	} else {
		$qryFT = "SELECT T1.*, (T2.company_name) as empresa, (T3.status) as status FROM objects T1 INNER JOIN companies T2 ON T1.fk_company = T2.row_id INNER JOIN ordstatus T3 ON T1.fk_status = T3.row_id WHERE fk_obj_type = 1 ".$qryAdd.";";
	}
	// FIN DEL CAMBIO
	...
	if (mysql_num_rows($res)) {
		// COMIENZO DEL CAMBIO
		echo "<table><tbody><thead><tr><th width=\"7%\">Id</th><th width=\"10%\">Code</th><th width=\"20%\">Name</th><th width=\"20%\">Company</th><th width=\"8%\">Qty</th><th width=\"10%\">Status</th><th width=\"5%\">Price</th><th width=\"20%\">Actions</th></tr></thead>";
		
		while ($row = mysql_fetch_array($res)) {
			echo "<tr><td width=7%>";
			echo $row['row_id'].'</td>';
			echo '<td width="10%">'.$row['f_code'].'</td>';
			echo '<td width="20%">'.$row['f_name'].'</td>';
			echo '<td width="20%">'.$row['empresa'].'</td>';
			echo '<td width="8%">'.$row['qty'].'</td>';
			echo '<td width="10%">'.$row['status'].'</td>';
			echo '<td width="5%">'.$row['ppc'].'</td>';
			// FIN DEL CAMBIO
			echo '<td width="10%">';
