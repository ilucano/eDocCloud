<?php
	...
	// COMIENZO DEL CAMBIO
	if ($txtSearch!="") {
		$qryAdd = " WHERE T1.fk_barcode LIKE '%".$txtSearch."%' OR T2.company_name LIKE '%".$txtSearch."%' OR T3.f_code LIKE '%".$txtSearch."%' OR T3.f_name LIKE '%".$txtSearch."%'";
	}
	$qryCnt = "SELECT COUNT(*) as num FROM pickup T1 INNER JOIN companies T2 ON T1.fk_company = T2.row_id INNER JOIN objects T4 ON T1.fk_box = T4.row_id LEFT JOIN objects T3 ON T1.fk_order = T3.row_id".$qryAdd;
	// FIN DEL CAMBIO
	...
	if ($total_pages>$limit || $pagAct > 0) {
		if ($pagAct==0) {
			$qryFT = "SELECT T1.*, (T2.company_name) as empresa, CONCAT(T3.f_code, ' ', T3.f_name) as orden , CONCAT(T4.f_code, ' ', T4.f_name) as caja FROM pickup T1 INNER JOIN companies T2 ON T1.fk_company = T2.row_id INNER JOIN objects T4 ON T1.fk_box = T4.row_id LEFT JOIN objects T3 ON T1.fk_order = T3.row_id ".$qryAdd." LIMIT ".($pagAct * $limit).",".($limit).";";
		} else {
			$qryFT = "SELECT T1.*, (T2.company_name) as empresa, CONCAT(T3.f_code, ' ', T3.f_name) as orden , CONCAT(T4.f_code, ' ', T4.f_name) as caja  FROM pickup T1 INNER JOIN companies T2 ON T1.fk_company = T2.row_id INNER JOIN objects T4 ON T1.fk_box = T4.row_id LEFT JOIN objects T3 ON T1.fk_order = T3.row_id ".$qryAdd." LIMIT ".(($pagAct) * $limit).",".($limit).";";
		}
	} else {
		$qryFT = "SELECT T1.*, (T2.company_name) as empresa, CONCAT(T3.f_code, ' ', T3.f_name) as orden, CONCAT(T4.f_code, ' ', T4.f_name) as caja  FROM pickup T1 INNER JOIN companies T2 ON T1.fk_company = T2.row_id INNER JOIN objects T4 ON T1.fk_box = T4.row_id LEFT JOIN objects T3 ON T1.fk_order = T3.row_id  ".$qryAdd.";";
	}
	// FIN DEL CAMBIO
