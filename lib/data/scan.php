...
<div class="row">
	...
	//require $arrIni['base'].'lib/data/dScan.php'; 
		...
		function buscar(vId, vPag, vVar) {
			if (vId=="grill")
			{
				if (vVar=="pagina") {
					$.ajax({
					   type: "GET",
					   url: "http://<?php echo $_SERVER['SERVER_NAME'];?>/lib/data/dScan.php",
					   data: "pagAct="+vPag,
			...
			} else if (vId=="action")
			{
				$.ajax({
				   type: "GET",
				   url: "http://<?php echo $_SERVER['SERVER_NAME'];?>/lib/data/wf.scan.php",
