...
switch ($action)
{
	case "listyear":
		...
		foreach($row as $list)
		{
			...
			$list_result .= '<li><a data-list-year="'.$list['file_year'].'" href="#">'. $file_year_label .' ('.$file_count.')</a></li>';
...
function dropDownYearButton($row_id, $file_year)
{
	...
	$drop_down_list = '<ul style="max-height: 50px;" id="year-drop'.$row_id.'" data-dropdown-content class="f-year-dropdown f-dropdown" aria-hidden="true" tabindex="-1">'.$drop_down_list.'</ul>';
	
	
	return '<button id="set-year-button'.$row_id.'" href="#" data-dropdown="year-drop'.$row_id.'" aria-controls="drop'.$row_id.'" aria-expanded="false" class="'.$disabled_class.'tiny button dropdown">'.$file_year.'</button><br>' .$drop_down_list;
